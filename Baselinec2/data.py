...
class Corpus(object):
    ...
    def __init__(self, path):
        ...
        self.valid = self.tokenize(os.path.join(path, 'valid.txt'))		#All of the validation examples
        self.test  = self.tokenize(os.path.join(path, 'test.txt'))		#and all of the testing examples
    ...
    def tokenize(self, path):
        ...
        with open(path, 'r') as f:										#open the file
            ids = torch.LongTensor(tokens)								#Define a tensor to store all the ids that we get from the file
